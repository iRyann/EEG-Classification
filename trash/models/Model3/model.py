...
def diagnostic_data_quality(X, y, class_names=['Left Hand', 'Right Hand', 'Feet', 'Tongue']):
    ...
    fisher_ratio = between_class_variance / (within_class_variance + 1e-6)
...
def create_improved_3dclmi_model(input_shape, n_classes=4):
    ...
    """
    print(f"=== MODÈLE 3D-CLMI CORRIGÉ ===")
    ...
    # Convolutions spatio-temporelles progressives
    conv1 = Conv3D(32, kernel_size=(3, 3, 1), activation='relu', 
                   padding='same', kernel_regularizer=l2(0.001))(inputs)
    conv1 = BatchNormalization()(conv1)
    conv1 = Dropout(0.25)(conv1)
    pool1 = MaxPooling3D(pool_size=(2, 2, 1))(conv1)
    
    conv2 = Conv3D(64, kernel_size=(3, 3, 1), activation='relu', 
                   padding='same', kernel_regularizer=l2(0.001))(pool1)
    conv2 = BatchNormalization()(conv2)
    conv2 = Dropout(0.25)(conv2)
    pool2 = MaxPooling3D(pool_size=(2, 2, 1))(conv2)
    
    # Convolution spatiale finale
    conv3 = Conv3D(128, kernel_size=(3, 3, n_channels), activation='relu', 
                   padding='valid', kernel_regularizer=l2(0.001))(pool2)
    conv3 = BatchNormalization()(conv3)
    conv3 = Dropout(0.3)(conv3)
    
    # Aplatissement de la sortie CNN
    cnn_flattened = Flatten()(conv3)
    
    # === BRANCHE LSTM CORRIGÉE ===
    # CORRECTION 7: Reshape correct pour séquence temporelle
    # Moyenner sur les canaux pour créer une séquence temporelle par fréquence
    lstm_input = tf.keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=-2))(inputs)  # (batch, n_freqs, n_times)
    lstm_input = Reshape((n_freqs, n_times))(lstm_input)
    
    # LSTM bidirectionnel pour capturer les dépendances temporelles
    lstm1 = Bidirectional(LSTM(64, return_sequences=True, 
                              dropout=0.3, recurrent_dropout=0.3))(lstm_input)
    lstm2 = Bidirectional(LSTM(32, dropout=0.3, recurrent_dropout=0.3))(lstm1)
    
    # === FUSION INTELLIGENTE ===
    # CORRECTION 8: Mécanisme d'attention pour pondérer les branches
    cnn_dense = Dense(128, activation='relu')(cnn_flattened)
    lstm_dense = Dense(128, activation='relu')(lstm2)
    
    # Calcul des poids d'attention
    attention_input = Concatenate()([cnn_dense, lstm_dense])
    attention_weights = Dense(2, activation='softmax', name='attention')(attention_input)
    
    # Application des poids d'attention
    attention_weights_reshaped = tf.keras.layers.Lambda(lambda x: tf.reshape(x, (-1, 2, 1)))(attention_weights)
    cnn_weighted = Multiply()([cnn_dense, attention_weights_reshaped[:, 0, :]])
    lstm_weighted = Multiply()([lstm_dense, attention_weights_reshaped[:, 1, :]])
    
    # Fusion finale
    fused = Add()([cnn_weighted, lstm_weighted])
    fused = BatchNormalization()(fused)
    fused = Dropout(0.5)(fused)
    
    # === CLASSIFICATION ===
    dense1 = Dense(64, activation='relu', kernel_regularizer=l2(0.001))(fused)
...
def main_fix_model():
    ...
    try:
        ...
        # 5. ÉVALUATION
        metrics = enhanced_evaluation(model, X_val, y_val)
